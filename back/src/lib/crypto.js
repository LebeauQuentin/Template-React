...
export async function hash(password) {
  ...
  const { saltLength, hashLength, cost, blockSize, parallelization, maxmem } = auth.crypto.scrypt;
...
export async function compare(plainTextPassword, hashedPassword) {
  ...
  const { hashLength, cost, blockSize, parallelization, maxmem } = auth.crypto.scrypt;
...
export function unsaltedHash(data) {
  const { unsaltedHashAlgorithm } = auth.crypto;
